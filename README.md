...
⚙️ Налаштування Telegram (BotFather)

Щоб у користувача з'явилася кнопка "Меню" та швидка команда /start, виконайте наступне:

    Зайдіть у чат з @BotFather.

    Надішліть команду '/mybots' і оберіть потрібного бота.

    Оберіть варіант Edit Commands.

    Надішліть наступний текст:
    start - Почати розмову

    Тепер у лівому нижньому куті чату з ботом з'явиться синя кнопка Menu.
